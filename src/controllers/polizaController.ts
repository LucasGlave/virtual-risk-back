...
const editPolizaByNumber = async (req: Request, res: Response) => {
  try {
    const response = await polizaService.editPolizaByNumber(
      req.params.poliza,
      req.body
    );
    if (!response) {
...
const deletePolizaByNumber = async (req: Request, res: Response) => {
  try {
    const response = await polizaService.deletePolizaByNumber(
      req.params.poliza
    );
    if (!response) {
      return res.status(400).send("Poliza not found");
...
const filterPolizas = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (req.query.riesgo) filters.riesgo = req.query.riesgo as string;
    ...
    if (req.query.vigenciaInicio)
      filters.vigenciaInicio = new Date(req.query.vigenciaInicio as string);
    if (req.query.vigenciaFin)
      filters.vigenciaFin = new Date(req.query.vigenciaFin as string);
